...
以下の手動テストを、もう少し手数を少なく最適化できませんか?

## PR #16 手動テスト (fix-melpa-review-feedback)

PR #16 の変更は keymap 初期化箇所の移動と lambda/quote 形式の修正のみ。
**回帰リスクのポイントは「keymap が populated されているか」「3つのラムダが実行されるか」の2点だけ**なので、
1つの連続操作の中で両方が検証できるシナリオに集約した。

事前に `global-sekka-mode` を有効にしておくこと。

sekka の SKK 風変換フロー（重要）:
- **1回目の C-j**: ローマ字を変換し**自動確定**（フェンス内に第1候補が入る。候補選択モードには入らない）
- **2回目の C-j**（確定直後のカーソル位置で押す）: **候補選択モードに突入**し `sekka-select-mode-map` のキーが有効化される
- **小文字のみ** (`nihongo`) → ひらがな変換 / **先頭大文字** (`Nihongo`) → 漢字変換

| # | 操作 | 期待結果 | 検証対象 |
|---|------|---------|---------|
| T1 | `Nihongo` + `C-j` | 「日本語」でフェンス内に自動確定 | `sekka-mode-map` C-j、`sekka-henkan-region` |
| T2 | T1 の状態から `C-j` → `SPC` を数回 → `C-m` | 候補選択モード突入 → 候補が切り替わる → 確定 | `sekka-select-mode-map` SPC/C-m、spacekey lambda |
| T3 | T1 の状態から `C-j` → `C-g` | キャンセルされ最初の候補に戻る | `sekka-select-mode-map` C-g |
| T4 | T1 の状態から `C-j` → `C-u` | ひらがな「にほんご」に切り替わる | `sekka-select-mode-map` の非ナビゲーションキー |
| T5 | `ascii` + `M-j` | 先頭が大文字化され `Ascii` になる | `sekka-mode-map` M-j (`sekka-capitalize-trans`) |

> T2 で keymap の「複数キー」と spacekey lambda を確認できる。
> muhenkan lambda およびスティッキーキーは `(lambda ...)` 形式を修正しただけなので、バイトコンパイル通過をもって動作保証とする（手動テスト省略）。
